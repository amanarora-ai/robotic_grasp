...
def default_config() -> config_dict.ConfigDict:
    cfg = config_dict.create(
        ...
        # Minimal-lift variant: drop grasp_ok factor from r_lift and use sqrt(h*t).
        # Policy must discover grasping implicitly via lift reward alone. held_high
        # still gated by r_grasp > 0.3 (Option A) to prevent knock-up gaming.
        lift_no_grasp_ok=False,
...
class VegaPickYCB(VegaBase):
    ...
    def __init__(
        ...
        # Fingertip geom IDs (one collision geom per fingertip body) and object geom ID.
        # Used for filtering contact forces to (fingertip, object) pairs only.
        tip_body_ids = np.array([
            self._mj_model.site_bodyid[sid] for sid in self._fingertip_sites
        ])
        self._fingertip_geoms = jp.asarray(
            [int(self._mj_model.body_geomadr[bid]) for bid in tip_body_ids],
            dtype=jp.int32,
        )  # (5,)
        self._obj_geom = jp.asarray(
            int(self._mj_model.geom("object_geom").id), dtype=jp.int32
        )
    ...
    def _get_reward(
        ...
        # Continuous scoring: soft proximity and soft force replace hard thresholds.
        # Gradient exists everywhere; no flat zero region to escape from.
        _PROX_CAP  = 0.08   # per-finger: 0 at 8cm+, 1.0 at contact (linear)
        _FORCE_CAP = 0.5    # per-finger: 0 at 0 N, 1.0 at 0.5+ N (linear)
        near_score  = jp.clip(1.0 - tip_dists / _PROX_CAP, 0.0, 1.0)
        force_score = jp.clip(touch / _FORCE_CAP, 0.0, 1.0)
        r_obj_touch = jp.mean(1.0 - jp.tanh(10.0 * tip_dists))
        palm_preshape = (palm_to_obj_dist < _PALM_GRASP_THRESH).astype(jp.float32)

        # Cube-only contact filtering: for each fingertip, is there an active
        # (fingertip_geom, object_geom) contact pair? This filters out self-contact,
        # palm-contact, and table-contact — only true cube contacts count toward force.
        c1 = data.contact.geom[:, 0]                                  # (ncon,)
        c2 = data.contact.geom[:, 1]                                  # (ncon,)
        tips = self._fingertip_geoms[None, :]                         # (1, 5)
        c1b  = c1[:, None]                                            # (ncon, 1)
        c2b  = c2[:, None]                                            # (ncon, 1)
        pair_is_finger_cube = (((c1b == tips) & (c2b == self._obj_geom))
                               | ((c2b == tips) & (c1b == self._obj_geom)))  # (ncon, 5)
        # Active contacts only (dist ≤ 0 means penetration / contact)
        active = (data.contact.dist <= 0.0)[:, None]                  # (ncon, 1)
        finger_cube_contact = jp.any(pair_is_finger_cube & active, axis=0).astype(jp.float32)  # (5,)

        # Separated proximity (no force) and force (cube-only)
        n_near  = jp.sum(near_score)                                                  # 0-5
        n_force = jp.sum(force_score * finger_cube_contact)                           # 0-5
        # Proximity: palm-gated (prevents degenerate "one stretched finger near cube" grasps)
        r_grasp_prox = palm_preshape * jp.clip(n_near  / 3.0, 0.0, 0.5)
        # Force: no palm gate — cube contact already implies the hand is on the object
        r_force      = jp.clip(n_force / 3.0, 0.0, 0.5)
        r_grasp = r_grasp_prox + r_force
        # Back-compat: r_grasp_contact name reused downstream by lift's grasp_ok gate
        r_grasp_contact = r_force

        # Fixed lift/success targets — not curriculum-driven.
        _LIFT_FIXED_TARGET = 0.05   # 5 cm
        _LIFT_FIXED_STEPS  = 5.0    # 5 consecutive held steps

        # 5) Lift: continuous in height and time held.
        # If LIFT_NO_GRASP_OK is set (env var), drop grasp from BOTH grasp_ok factor
        # AND held_high threshold — lift becomes pure height × time.
        grasp_ok = jp.clip(r_grasp_contact / 0.25, 0.0, 1.0)
        height_factor = jp.clip(obj_height / _LIFT_FIXED_TARGET, 0.0, 1.0)
        if os.environ.get("LIFT_NO_GRASP_OK") == "1":
            # Height-only gate for held_steps — no r_grasp requirement
            held_high = (obj_height > _LIFT_FIXED_TARGET).astype(jp.float32)
        else:
            held_high = ((obj_height > _LIFT_FIXED_TARGET) & (r_grasp > 0.3)).astype(jp.float32)
        info["held_steps"] = (info["held_steps"] + 1.0) * held_high
        time_factor = jp.clip(info["held_steps"] / _LIFT_FIXED_STEPS, 0.0, 1.0)
        # Geometric mean of the active factors.
        if os.environ.get("LIFT_NO_GRASP_OK") == "1":
            r_lift = jp.sqrt(height_factor * time_factor)
        else:
            r_lift = jp.cbrt(grasp_ok * height_factor * time_factor)

        # 6) Hold: unchanged — fires whenever grasp active and object above 1 cm
        grasp_active = (r_grasp > 0.3).astype(jp.float32)
        r_hold = grasp_active * jp.clip(obj_height / _HOLD_GATE, 0.0, 1.0)

        # 7) Success: binary, fixed at 5cm + 5 steps (no curriculum)
        r_success = (info["held_steps"] >= _LIFT_FIXED_STEPS).astype(jp.float32)
        ...
        rewards = {
            ...
            # Diagnostics (filtered out of reward sum via step()). Raw values, no scales.
            "diag_grasp_ok":       grasp_ok,
            "diag_height_factor":  height_factor,
            "diag_time_factor":    time_factor,
            "diag_held_steps":     info["held_steps"],
            "diag_palm_preshape":  palm_preshape,
            "diag_near_score":     jp.mean(near_score),
            "diag_force_score":    jp.mean(force_score),
            "diag_r_grasp_prox":   r_grasp_prox,
            "diag_r_grasp_contact": r_grasp_contact,
            "diag_r_force":        r_force,
            "diag_finger_cube_contact": jp.sum(finger_cube_contact),  # 0-5: how many fingers touching cube
        }
        return rewards
    ...
    def reset(self, rng: jax.Array) -> State:
        ...
        # Add diagnostic metric keys so step() outputs match scan's initial carry pytree
        for _k in ("diag_grasp_ok", "diag_height_factor", "diag_time_factor",
                   "diag_held_steps", "diag_palm_preshape", "diag_near_score",
                   "diag_force_score", "diag_r_grasp_prox", "diag_r_grasp_contact",
                   "diag_r_force", "diag_finger_cube_contact"):
            metrics[_k] = jp.zeros(())
        info = {
            "rng": rng,
            "held_steps":      jp.zeros(()),
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        # Exclude diagnostic entries (keys starting with "diag_") from reward sum
        reward_sum = sum(v for k, v in raw_rewards.items() if not k.startswith("diag_"))
        reward = jp.clip(reward_sum + fall_penalty, -1e4, 1e4)
